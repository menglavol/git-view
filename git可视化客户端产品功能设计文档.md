...
点击仓库后，右侧抽屉展示详情，分「基本信息」与「提交历史」两个 Tab。
...
提交历史 Tab（V1，GitHub / GitLab / Gitee 通用）：

1. 分页加载该远程仓库的提交，滚动到底自动加载下一页
2. 列表项：commit hash、message、author、date
3. 点击提交查看提交详情：完整 message、作者 / 时间 / SHA、增删统计
4. 提交详情含改动文件列表与逐文件着色 diff
5. 跳转平台对应的提交网页
...
4. date
5. branch / tag 标签
6. 点击提交查看 commit 详情：完整 message、作者 / 时间 / SHA、增删统计
7. 查看 commit diff：改动文件列表 + 逐文件着色 diff

> 「查看 commit 详情 / diff」原规划属 V2，V1 已提前实现，并与远程仓库「提交历史」共用同一详情视图。
...
管理数据库与日志文件的存放位置，位于通用设置底部：

1. 显示当前数据目录，只读
2. 更改目录：选新目录后复制数据库与日志迁移过去，需重启生效，旧目录保留
3. 删除旧目录：迁移后永久删除旧数据目录，二次确认
4. 恢复默认目录：把数据复制回应用默认位置，需重启生效
...
设置页通用 Tab 底部展示日志文件占用并提供清理：

1. 显示日志目录路径、占用大小、日志文件数，可刷新重新统计
2. 清理历史日志：删除当天之前的日志文件、仅保留当天，二次确认，提示释放空间
3. 区别于 §15.2 操作日志（数据库记录），此处清理的是磁盘日志文件
